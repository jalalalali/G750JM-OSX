 ...
 * 
 * Disassembly of iASL1DvkPv.aml, Thu Oct 30 18:33:20 2014
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00014E41 (85569)
 *     Revision         0x02
 *     Checksum         0x1A
 *     OEM ID           "_ASUS_"
 ...
 */
DefinitionBlock ("iASL1DvkPv.aml", "DSDT", 2, "_ASUS_", "Notebook", 0x00000012)
{
    /*
    ...
    External (_SB_.PCI0.IGPU.PBCC, UnknownObj)
    External (_SB_.PCI0.IGPU.PBCM, UnknownObj)
    External (_SB_.PCI0.IGPU.PBDI, UnknownObj)
    External (_SB_.PCI0.IGPU.PBMX, UnknownObj)
    ...
    External (_SB_.PCI0.PEG0.PEGP.ONOF, IntObj)
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0x7AB03C18, 0x02B2)
...
{If (LLessEqual (Arg0, 0x5E))
    ...
    Scope (_GPE)
    {
        ...
        Method (_E10, 0, NotSerialized)  // _Exx: Edge-Triggered GPE
        {
            Name (TSNE, 0x03)
            TBAC ()
            Acquire (OSUM, 0xFFFF)
    ...
    Scope (_SB.PCI0.LPCB.EC0)
    {
        ...
        Method (STBR, 0, Serialized)
        {
            ...
            If (Local1)
            {
                ...
                Name (TMP2, Zero)
                Name (LCD0, Zero)
                If (LEqual (^^^IGPU.PBMX, Zero))
                {
                    Store (0x02, ^^^IGPU.PBMX)
                    If (LEqual (^^^IGPU.PBMX, 0x02))
                    {
                        Store (Zero, Local0)
                        While (LAnd (LAnd (^^^IGPU.PBCC, 0xFF), LLess (Local0, 0x05)))
                        {
                            Increment (Local0)
                            Sleep (One)
                        }

                        If (LEqual (And (^^^IGPU.PBCM, 0x02), Zero))
                        {
                            Multiply (BRTL, 0x0A, TMP1)
                            ShiftLeft (TMP1, 0x08, TMP2)
                            Or (TMP2, LCD0, TMP1)
                            Store (TMP1, ^^^IGPU.PBDI)
                            Store (0x86100000, ^^^IGPU.PBCM)
                            Sleep (0x02)
                        }

                        Store (Zero, ^^^IGPU.PBMX)
                        Return (One)
                    }
                }
    ...
    Method (PTS, 1, NotSerialized)
    {
        If (Arg0)
        {
            ...
            \_SB.PCI0.IGPU.OPTS (Arg0, OEMS (Arg0))
        }
    }
    ...
    Method (WAK, 1, NotSerialized)
    {
        ...
        \_SB.ATKD.GENW (Arg0)
        \_SB.PCI0.IGPU.OWAK (Arg0, OEMW (Arg0))
    }
        ...
        Method (FCPU, 0, Serialized)
        {
            ...
            Else
            {
                ...
                If (LGreaterEqual (Local3, Local1))
                {
                    ...
                    \_SB.PCI0.LPCB.EC0.SFNV (One, FSPD)
                    \_SB.PCI0.LPCB.EC0.SFNV (0x02, FSPD)
                    Store (Zero, FCNT)
                }
            }
